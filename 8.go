...
func part1(){
	...
	re, _ := regexp.Compile(`([a-z]+)\s(dec|inc)\s(-?[0-9]+)\sif\s([a-z]+)\s(!=|<=|<|==|>|>=)\s(-?[0-9]+)`)
	...
	for a, v := range rows {
		...
		// Add any unknown registers
		if _, ok := registers[groups[1]]; !ok {
			registers[groups[1]] = 0
		}

		if _, ok := registers[groups[4]]; !ok {
			registers[groups[4]] = 0
		}

		exprIsTrue := false
		compRHS, _ := strconv.Atoi(groups[6])
		compLHS    := registers[groups[4]]

		switch groups[5]{
		case ">":
			exprIsTrue = compLHS > compRHS
		case "<":
			exprIsTrue = compLHS < compRHS
		case ">=":
			exprIsTrue = compLHS >= compRHS
		...
		fmt.Println(a)
		fmt.Println(exprIsTrue)

		if exprIsTrue {
			num, _ := strconv.Atoi(groups[3])
			if groups[2] == "inc" {
				registers[groups[1]] += num
			} else {
				registers[groups[1]] -= num
			}
		}

		//fmt.Println("Comparsion of ", v, " is ", exprIsTrue)
		//fmt.Println(groups[1], " = ", registers[groups[1]])
		fmt.Println(v)

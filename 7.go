...
import (
	...
	func getWeightOfNodeIncludingChildren(node *Node, tabs int) int {
		...
		fmt.Println(indent,"Processing Node ", node.name)
		...
		if node.isParent {
			...
			for i := 0; i < len(node.children); i++ {
				node.children[i].fullWeight = getWeightOfNodeIncludingChildren(node.children[i], tabs+1)
				weight += node.children[i].fullWeight
				//
				//if childWeight == -1 {
				//	childWeight = node.children[i].fullWeight
				//}
				//
				//if childWeight != node.children[i].fullWeight {
				//	foundUnbalanced = true
				//}
			}

			for i := 0; i < len(node.children); i++ {
				fmt.Println(indent, node.children[i].fullWeight, " ", node.children[i].name)
			}
			//
			//if foundUnbalanced {
			//	for i := 0; i < len(node.children); i++ {
			//		fmt.Println(node.children[i].name, " is unbalanced", node.children[i].fullWeight)
			//	}
			//}
		}

		weight += node.ownWeight

		//fmt.Println(indent, "weight = ", weight)
		return weight
	}
...
uhcach (18)`
		...
		re, _ := regexp.Compile(`([a-z]+)\s\(([0-9]+)\)(?:\s->\s)?(.*)`)

		for _, v := range rows {

			// Get the values out of the string
			out := re.FindStringSubmatch(v)
			weight, _ := strconv.Atoi(out[2])
			var n = Node{out[1], weight , out[3], nil, false, 0}

			// Create the list of node and those with children
			nodes[n.name] = &n
			if n.childrenText != "" {
				n.isParent = true
			}
		...
		// Construct the tree by processing those with children
		for _, parent := range nodes {

			if parent.isParent {
				childrenNames := strings.Split(parent.childrenText, ", ")
				parent.children = make([]*Node, len(childrenNames))
				for i := 0; i < len(childrenNames); i++ {
					parent.children[i] = nodes[childrenNames[i]]
				}
			}
		}

		// Walk the tree calculating the weight

		root := nodes["mkxke"]
		root.fullWeight = getWeightOfNodeIncludingChildren(root, 0)
	}